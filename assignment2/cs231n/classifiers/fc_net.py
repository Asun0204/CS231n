...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        out, cache1 = affine_relu_forward(X.reshape(X.shape[0], np.prod(X.shape[1:])), self.params['W1'], self.params['b1'])
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for idx in xrange(self.num_layers-1):
            ...
            if self.use_batchnorm:
                dout, grads['W%d'%(lay+1)], grads['b%d'%(lay+1)], grads['gamma%d'%(lay+1)], grads['beta%d'%(lay+1)] = affine_bn_relu_backward(dout, ar_cache[lay])
            else:
                dout, grads['W%d'%(lay+1)], grads['b%d'%(lay+1)] = affine_relu_backward(dout, ar_cache[lay])
