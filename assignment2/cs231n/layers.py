...
def max_pool_forward_naive(x, pool_param):
    ...
    H_pool = (H - FH) / stride + 1
    W_pool = (W - FW) / stride + 1

    out = np.zeros((N, C, H_pool, W_pool))
    for i in xrange(H_pool):
        for j in xrange(W_pool):
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in xrange(H_pool):
        for j in xrange(W_pool):
            ...
            dx[:, :, i*stride:(i+1)*stride, j*stride:(j+1)*stride] = (dout[:, :, i, j])[:, :, None, None] * x_mask_binary
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
